...
export function sendGlobal(key: string, value: unknown) {
  !isNull(global.sharedObject)
    ? (global.sharedObject[key] = value)
    : ipcRenderer.sendSync('global-sharedObject-set', {
        key,
        value
      });
}
...
export function getGlobal(key: string) {
  return !isNull(global.sharedObject)
    ? global.sharedObject[key]
    : ipcRenderer.sendSync('global-sharedObject-get', {
        key
      });
}
